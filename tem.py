...
df=pd.read_csv(r'C:\Users\DELL\OneDrive\Desktop\luffy\proj3\proj3.csv')
...
if select == "Home":
    image1=Image.open(r'C:\Users\DELL\OneDrive\Desktop\luffy\proj3\airbnb-logo-0.webp')
    st.image(image1)
...
if select == "Data Exploration":
    ...
    with tab2:
        ...
        def datafr():
            df_a=pd.read_csv(r'C:\Users\DELL\OneDrive\Desktop\luffy\proj3\proj3.csv')
            return df_a
    ...
    with tab3:
        ...
        # Load the DataFrame (replace this with actual data loading code)
        # For example: df_val_sel = pd.read_csv("your_data.csv")
        # Ensure df_val_sel is loaded and defined before proceeding
        df_val_sel = pd.read_csv(r'C:\Users\DELL\OneDrive\Desktop\luffy\proj3\proj3.csv')  # replace with your data loading method

        # Checking the correlation
        df_val_sel_corr = df_val_sel.drop(columns=[
            ...
            "government_area", "market", "country", "country_code", "location_type", "is_location_exact",
            "amenities"
        ]).corr()

        st.dataframe(df_val_sel_corr)

        # Grouping by "accommodates"
        df_val_sel_gr = df_val_sel.groupby("accommodates")[["cleaning_fee", "bedrooms", "beds", "extra_people"]].sum()
        df_val_sel_gr.reset_index(inplace=True)

        # Plot for accommodates
        ...
        # Filter dataframe by selected room type
        df_val_sel_rt = df_val_sel[df_val_sel["room_type"] == room_ty_l]

        # Plot for selected room type
        fig_2 = px.bar(df_val_sel_rt, x=["street", "host_location", "host_neighbourhood"], y="market", 
                    title="MARKET", hover_data=["name", "host_name", "market"], barmode='group', 
                    orientation='h', color_discrete_sequence=px.colors.sequential.Rainbow_r, width=1000)
        st.plotly_chart(fig_2)

        # Revised Plot for government area
        # Convert 'host_is_superhost' from categorical to numerical if necessary
        df_val_sel_rt['host_is_superhost_num'] = df_val_sel_rt['host_is_superhost'].apply(lambda x: 1 if x == 't' else 0)

        # Ensure 'cancellation_policy' is numerical or handle it appropriately for plotting
        # If 'cancellation_policy' is categorical, we can't plot it directly with numerical data
        # Let's assume 'cancellation_policy' needs to be counted by 'government_area'

        # Count occurrences of each cancellation policy per government area
        df_cancellation_policy_count = df_val_sel_rt.groupby(['government_area', 'cancellation_policy']).size().reset_index(name='counts')

        # Plot superhost status by government area
        fig_superhost = px.bar(df_val_sel_rt, x="government_area", y="host_is_superhost_num", 
                            title="Superhost Status by Government Area", 
                            hover_data=["guests_included", "location_type"], 
                            barmode='group', color_discrete_sequence=px.colors.sequential.Rainbow_r, width=1000)
        st.plotly_chart(fig_superhost)

        # Plot cancellation policy counts by government area
        fig_cancellation_policy = px.bar(df_cancellation_policy_count, x="government_area", y="counts", 
                                        color="cancellation_policy", title="Cancellation Policy by Government Area", 
                                        barmode='group', color_discrete_sequence=px.colors.sequential.Rainbow_r, width=1000)
        st.plotly_chart(fig_cancellation_policy)
    with tab4:
